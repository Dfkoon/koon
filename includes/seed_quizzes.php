...
function seed_quizzes_from_bundle(PDO $db, bool $force = false): array
{
    ...
    // 1. فحص النسخ الاحتياطية السابقة في مجلد .backups أولاً
    $backupFiles = glob($baseDir . '/.backups/*.sqlite');
    if (!empty($backupFiles)) {
        rsort($backupFiles);
        foreach ($backupFiles as $bFile) {
            try {
                $bDb = new PDO('sqlite:' . $bFile);
                $bDb->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                $hasTable = (int) $bDb->query("SELECT count(*) FROM sqlite_master WHERE type='table' AND name='quiz_questions'")->fetchColumn();
                if ($hasTable > 0) {
                ...
                error_log('Error inspecting backup ' . $bFile . ': ' . $be->getMessage());
            }
        }
    }
